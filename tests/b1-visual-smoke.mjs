...
const server = http.createServer(async (request, response) => {
  try {
    ...
    if (!pathname.startsWith("/gsi-b1/")) { response.writeHead(404).end(); return; }
...
const base = `http://127.0.0.1:${server.address().port}/gsi-b1/index.html`;
const manifest = JSON.parse(await fs.readFile(path.join(root, "content/enhancements/b1-visuals.json"), "utf8"));
const browser = await chromium.launch({ channel: "chrome", headless: true });
const results = [];
const failures = [];
const consoleErrors = [];
const broken = [];
const output = path.join(root, "tmp/gsi-b1");
...
try {
  for (const mode of [
    ...
    for (let number = 1; number <= 10; number += 1) {
      const topicId = `B1-T${String(number).padStart(2, "0")}`;
      try {
        ...
        await page.waitForFunction(() => [...document.querySelectorAll(".topic-visual img")].every((image) => image.complete && image.naturalWidth > 0));
        const metrics = await page.evaluate(() => ({
          viewport: innerWidth,
          scrollWidth: document.documentElement.scrollWidth,
          visuals: [...document.querySelectorAll(".topic-visual img")].map((image) => ({
            naturalWidth: image.naturalWidth,
            naturalHeight: image.naturalHeight,
            renderedWidth: Math.round(image.getBoundingClientRect().width),
            containerWidth: Math.round(image.closest(".topic-visual-block").getBoundingClientRect().width),
        ...
        if (mode.name === "desktop") {
          assert.ok(metrics.visuals.every((item) => item.renderedWidth <= item.containerWidth + 1), `${topicId}: cropped visual`);
        } else {
          assert.ok(metrics.visuals.every((item) => item.renderedWidth > item.containerWidth), `${topicId}: mobile diagram did not preserve legible width`);
        }
        assert.ok(metrics.visuals.every((item) => item.altLength >= 60), `${topicId}: insufficient alt text`);
        for (let index = 0; index < expected; index += 1) {
          await page.locator(".topic-visual-block").nth(index).screenshot({ path: path.join(output, `${topicId}-${mode.name}-visual-${index + 1}.png`) });
        }
        await page.screenshot({ path: path.join(output, `${topicId}-${mode.name}.png`), fullPage: true });
  ...
  failures.push({ topicId: "global", mode: "all", error: error.message });
} finally {
  await fs.writeFile(path.join(root, "logs/b1-visual-smoke.json"), JSON.stringify({ date: new Date().toISOString(), results, failures, consoleErrors, broken }, null, 2) + "\n");
  await browser.close();
