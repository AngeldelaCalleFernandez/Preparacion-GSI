...
const server = http.createServer(async (request, response) => {
  try {
    ...
    if (!pathname.startsWith("/gsi-b2/")) { response.writeHead(404).end(); return; }
...
const base = `http://127.0.0.1:${server.address().port}/gsi-b2/index.html`;
const manifest = JSON.parse(await fs.readFile(path.join(root, "content/enhancements/b2-visuals.json"), "utf8"));
const browser = await chromium.launch({ channel: "chrome", headless: true });
const results = [];
const failures = [];
const consoleErrors = [];
const broken = [];
const output = path.join(root, "tmp/gsi-b2");
...
try {
  for (const mode of [
    ...
    for (let number = 1; number <= 16; number += 1) {
      const topicId = `B2-T${String(number).padStart(2, "0")}`;
      try {
  ...
  await fs.writeFile(path.join(root, "logs/b2-visual-smoke.json"), JSON.stringify({ date: new Date().toISOString(), results, failures, consoleErrors, broken }, null, 2) + "\n");
  await browser.close();
