...
const server = http.createServer(async (request, response) => {
  try {
    ...
    if (!pathname.startsWith("/gsi-b3/")) { response.writeHead(404).end(); return; }
...
const base = `http://127.0.0.1:${server.address().port}/gsi-b3/index.html`;
const manifest = JSON.parse(await fs.readFile(path.join(root, "content/enhancements/b3-visuals.json"), "utf8"));
const browser = await chromium.launch({ channel: "chrome", headless: true });
const results = [], failures = [], consoleErrors = [], broken = [];
const output = path.join(root, "tmp/gsi-b3");
...
try {
  for (const mode of [{ name: "desktop", width: 1365, height: 900 }, { name: "mobile", width: 390, height: 844 }]) {
    ...
    for (let number = 1; number <= 15; number += 1) {
      const topicId = `B3-T${String(number).padStart(2, "0")}`;
      try {
  ...
  await fs.writeFile(path.join(root, "logs/b3-visual-smoke.json"), JSON.stringify({ date: new Date().toISOString(), results, failures, consoleErrors, broken }, null, 2) + "\n");
  await browser.close();
