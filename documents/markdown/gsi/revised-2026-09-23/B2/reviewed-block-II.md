...
La fecha de control de actualidad de esta edición es 23/09/2026.
...
Organiza software alrededor de objetos que combinan estado y comportamiento. Conceptos: clase, objeto, encapsulación, abstracción, herencia, polimorfismo, composición e interfaces. Java, C#, C++ y muchos lenguajes modernos son multiparadigma y ofrecen OO. Kotlin es un lenguaje moderno diseñado con interoperabilidad con Java/JVM, por lo que puede convivir con código y bibliotecas Java.
...
Los sistemas modernos estructuran código en módulos/paquetes y gestionan dependencias con repositorios y versiones. Las excepciones separan flujo normal y manejo de errores, aunque algunos lenguajes emplean tipos de resultado. Los genéricos permiten reutilizar algoritmos manteniendo información de tipos. En Python, la importación programática/dinámica se realiza habitualmente mediante importlib.import\_module(); importlib.machinery.SourceFileLoader sigue existiendo como cargador concreto de código fuente, pero su método load\_module está deprecado en favor del mecanismo moderno basado en exec\_module().
...
# Microdatos oficiales añadidos — PHP, C y tipos algebraicos

# Dato de test INAP 2024: para PHP se dio por correcta la combinación de paradigmas imperativo, orientado a objetos, funcional y reflexivo. En C, una constante es una entidad cuyo valor no puede ser modificado por el programa. Dato OEP 2025 provisional: los tipos algebraicos (sum/product types según lenguaje) permiten representar variantes o estados posibles de manera exhaustiva y segura, favoreciendo comprobaciones de tipos y pattern matching.
...
**Actualización 23/09/2026: Kubernetes 1.37 fue publicada el 26/08/2026 y el proyecto mantiene como ramas menores mantenidas 1.37, 1.36 y 1.35. Para examen se priorizan objetos y arquitectura estables; no compensa memorizar números de parche.**
...
**Imagen es una plantilla inmutable por capas; contenedor es una instancia en ejecución. Un registry almacena y distribuye imágenes: puede ser público cuando el artefacto está destinado a distribución abierta —por ejemplo, proyectos open source— o privado cuando el acceso debe restringirse. OCI define especificaciones interoperables para imágenes y runtimes. En Docker, los volúmenes son el mecanismo preferido para persistir datos generados o utilizados por contenedores; la capa escribible del contenedor no debe tratarse como almacenamiento persistente.**
...
CI/CD = Continuous Integration / Continuous Delivery (o Continuous Deployment cuando se automatiza también el paso a producción según el modelo). La CI integra cambios y crea/valida artefactos; la entrega/despliegue automatiza su promoción. GitOps utiliza un repositorio declarativo como fuente de estado deseado y un reconciliador aplica cambios.
...
RBAC de mínimo privilegio, aislamiento por namespaces/políticas de red, protección del API, imágenes verificadas, escaneo, secrets externos cuando proceda, admission policies, runtime security, logs y actualizaciones. Proteger la cadena de suministro desde repositorio hasta registry y clúster. En Kubernetes, spec.hostPID: true hace que un Pod use el namespace PID del host, reduciendo el aislamiento; las políticas Baseline/Restricted impiden compartir namespaces del host.
...
| --- | --- | --- |
| 7 Aplicación | Servicios a aplicaciones | HTTP, DNS, SMTP, SNMP, RTSP |
| 6 Presentación | Representación, codificación, transformación | formatos, serialización, cifrado conceptualmente |
| 5 Sesión | Diálogo/sesiones | control de diálogo/sesión, sincronización, checkpoints y recuperación |
...
Una dirección MAC identifica una interfaz en un dominio de enlace. Ethernet transporta tramas. Los switches aprenden asociaciones MAC↔puerto y reenvían tramas. Una VLAN crea dominios de broadcast lógicos separados. En la clasificación clásica de test, una VLAN creada mediante la asignación de puertos físicos de un switch a esa VLAN es una VLAN estática.
...
Dirección de 32 bits. CIDR expresa prefijo /n. La máscara separa red y host. Aunque el direccionamiento classful está superado por CIDR, sigue apareciendo en preguntas oficiales: clase A, primer octeto 1–126; clase B, 128–191; clase C, 192–223; 127 queda reservado para loopback. Ejemplos:
...
Los puertos identifican extremos lógicos de transporte. Un socket puede entenderse como combinación de protocolo, IP y puerto. IANA divide los números de puerto en System Ports 0-1023 —tradicionalmente llamados well-known—, User Ports 1024-49151 y Dynamic/Private 49152-65535. Valores a reconocer: SSH 22/TCP; DNS 53 UDP/TCP; HTTP 80/TCP; HTTPS 443 (TCP y QUIC/UDP según HTTP/3); SMTP 25; NTP 123/UDP; SNMP 161/162 UDP típicamente. SNMP significa Simple Network Management Protocol y es un protocolo de aplicación para monitorización/gestión de dispositivos de red. En la terminología clásica de la pregunta INAP 2019, el polling SNMP es monitorización activa y los traps/notificaciones recibidos sin sondeo se consideran monitorización pasiva. La IETF (Internet Engineering Task Force) es la principal organización de desarrollo de estándares de Internet y los RFC constituyen su producción técnica central. No memorices cientos de puertos sin prioridad.
...
Un router decide el siguiente salto utilizando tabla de rutas y la coincidencia de prefijo más específica. Rutas pueden ser conectadas, estáticas o aprendidas dinámicamente. La ruta por defecto se usa cuando no existe una más específica. En calidad de servicio conviene reconocer tres enfoques clásicos: Best Effort, IntServ (Integrated Services) y DiffServ (Differentiated Services); «PriServ» no es uno de esos modelos estándar.
...
# Microdato oficial añadido — wildcard IPv4

# La wildcard mask es la inversa bit a bit de la máscara de subred. Ejemplo de la OEP 2025 provisional: /17 → máscara 255.255.128.0 → wildcard 0.0.127.255. En el esquema classful histórico, una dirección cuyo primer octeto es 171 pertenece a clase B; para diseño actual se usa CIDR/prefijo, no clases.
...
**XML es un metalenguaje/sintaxis extensible para representar documentos y datos estructurados. Las etiquetas las define la aplicación. La especificación XML es responsabilidad del World Wide Web Consortium (W3C). Un documento bien formado cumple reglas sintácticas XML; uno válido además cumple el vocabulario/esquema declarado.**
...
## 8. XPath, XSLT y XLink

**XPath selecciona nodos/valores dentro de XML mediante expresiones de rutas. XSLT transforma documentos XML a XML, HTML, texto u otros resultados, utilizando XPath para seleccionar partes. XLink permite expresar enlaces y relaciones entre recursos/documentos mediante XML. Regla de test: XPath selecciona; XSLT transforma; XLink enlaza.**
...
# Microdato oficial añadido — XML Schema

# XML Schema (XSD) está escrito en XML, permite definir y restringir tipos de datos y se considera fuertemente tipado. DTD tiene una capacidad de tipado mucho más limitada. En la OEP 2024 definitiva, “XML Schema es fuertemente tipado” fue la opción correcta.
...
* **Impacto: consecuencia de materialización. En MAGERIT v3, el impacto acumulado se calcula a partir del valor acumulado del activo y de la degradación causada; por tanto, conocer solo el valor propio del activo y el porcentaje de degradación no basta para obtener el impacto acumulado si no se conoce su valor acumulado.**
...
Amenaza y vulnerabilidad no son intercambiables. Un servidor sin parche tiene una vulnerabilidad; un atacante o malware puede ser amenaza; el riesgo surge al combinar contexto, exposición y consecuencias. En MAGERIT, la dimensión de integridad se refiere al mantenimiento de la completitud y corrección de los datos.
...
**Cualitativo: escalas como bajo/medio/alto y matrices; rápido y comprensible, pero menos preciso. Un mapa de calor o matriz de riesgos representa habitualmente probabilidad/frecuencia frente a impacto para priorizar. Cuantitativo: intenta expresar frecuencias/pérdidas en magnitudes, útil cuando hay datos fiables. En la práctica existen enfoques semicuantitativos.**
...
El material oficial la relaciona con ISO 31000 y con la exigencia del ENS de gestionar seguridad basada en riesgos. Como referencias internacionales vigentes que conviene reconocer: ISO/IEC 27005:2022 proporciona guía para gestionar riesgos de seguridad de la información, y NIST SP 800-30 Rev.1 ofrece una guía para realizar evaluaciones de riesgos.
...
# Microdatos oficiales añadidos — CRM y multicanal

# En la clasificación habitual, CRM operacional automatiza procesos comerciales/servicio; CRM analítico explota datos para conocimiento y segmentación; CRM colaborativo coordina la interacción con el cliente entre canales y áreas. En la OEP 2024 definitiva, el CRM centrado en mejorar la interacción a través de múltiples canales fue el colaborativo.
...
* **Malware: virus, gusano, troyano, ransomware, spyware, rootkit, RAT. Una infraestructura o servidor de Command and Control (C&C/C2) coordina y envía órdenes a sistemas comprometidos o bots.**
...
* **Phishing/spear phishing: ingeniería social para credenciales/acción. Smishing es phishing realizado mediante SMS o mensajería de texto.**
...
* **INÉS: solución del CCN para gobernanza de la ciberseguridad, evaluación regular del estado de seguridad de sistemas TIC y su adecuación/implantación al ENS.**
...
Validar alerta, determinar alcance, activos/usuarios, vector, IoC, cronología, impacto y necesidad de notificaciones. Preservar evidencia desde el inicio. CCN-STIC 817 distingue precursor —indicio de que un incidente puede ocurrir en el futuro— de indicador —indicio de que puede haber ocurrido o estar ocurriendo ahora—.

Los Sistemas de Alerta Temprana del CCN-CERT incluyen SAT-INET, para detectar en tiempo real amenazas e incidentes en el tráfico entre la red interna del organismo e Internet, y SAT-ICS, orientado a redes de control y supervisión industrial. SAT-INET no se centra en el contenido del tráfico salvo en aquello relevante para detectar una amenaza. En CCN-STIC 817, la peligrosidad del ciberincidente se asigna en cinco niveles crecientes: bajo, medio, alto, muy alto y crítico.
...
## La European Union Public Licence (EUPL) es una licencia de software libre promovida por la Comisión Europea y especialmente relevante en Administraciones Públicas europeas. Es copyleft, está disponible en múltiples lenguas oficiales y fue diseñada teniendo en cuenta el marco jurídico de la UE. El ENI exige que las licencias de aplicaciones de fuentes abiertas garanticen ejecución para cualquier propósito, acceso al código fuente, modificación/mejora y redistribución —con o sin cambios— manteniendo esas garantías, y establece que para este fin se procurará aplicar la EUPL, sin excluir otras licencias que garanticen los mismos derechos. La compatibilidad con otras licencias debe comprobarse conforme a la versión y apéndice aplicables.
...
El art. 97.4 del Texto Refundido de la Ley de Propiedad Intelectual establece que, cuando un trabajador asalariado crea un programa de ordenador en el ejercicio de sus funciones o siguiendo instrucciones del empresario, los derechos de explotación del programa fuente y del programa objeto corresponden exclusivamente al empresario, salvo pacto en contrario. En contratación pública/externa, la cesión/licencia debe quedar expresamente regulada: código, documentación, derechos de modificación, reutilización y entregables.
...
Un DMS/gestor documental administra documentos y su ciclo de vida: captura, clasificación, metadatos, versiones, permisos, búsqueda, retención, archivo y disposición. En el sector público se relaciona con documento/expediente electrónico, ENI/NTI y política de gestión documental, tratados más jurídicamente en Bloque I. Estándares a reconocer: ISO 30300:2020, conceptos y vocabulario de records management; OAIS, modelo de referencia para sistemas de archivo abierto, cuya edición ISO vigente es ISO 14721:2025; y CMIS 1.1 de OASIS, estándar de interoperabilidad para repositorios/servicios de gestión de contenidos.
...
Herramientas: coedición, wikis, repositorios, chat/canales, videoconferencia, foros, gestión de tareas, pizarras y control de versiones. Deben gobernarse: identidad, permisos, ciclo de vida de espacios, clasificación, retención, invitados, exportaciones y búsqueda. Dato operativo vigente: en Microsoft Teams, los archivos de los canales estándar se almacenan en el sitio de SharePoint asociado al equipo; todos los canales estándar comparten ese sitio y cada canal se refleja como carpeta en su biblioteca de documentos.
...
# ANEXO D — Registro de ajustes V2.1 para Codex

Nota de auditoría: revisión del Bloque II contra preguntas oficiales GSI de 2018, 2019, 2022 y 2024 con plantilla definitiva, más OEP 2025 —ejercicio celebrado el 23/05/2026— con plantilla provisional. Solo se incorpora contenido que sigue siendo útil y vigente a 23/09/2026.

Cambio 01 — II.01: precisados fundamentos de arquitectura y computación distribuida.

Motivo/evidencia: preguntas oficiales 2018, 2019 y 2022.

Contenido: registro de instrucción; clasificación MIMD de Tanenbaum por grado de acoplamiento; edge computing como procesamiento próximo a fuente/usuario. Criterio: conceptos estables, no hardware coyuntural.

Cambio 02 — II.02: añadidos microdatos estables de sistemas operativos.

Motivo/evidencia: preguntas oficiales 2018 y 2019.

Contenido: planificador de largo plazo; algoritmos FIFO/NRU/LRU/NFU/LFU; /etc; permisos octales Unix. Criterio: reconocer reglas de test, sin convertir el tema en una lista de comandos.

Cambio 03 — II.05: reforzada precisión de SGBD/modelo relacional.

Motivo/evidencia: preguntas oficiales 2019, 2022 y 2024.

Contenido: TRUNCATE frente a DROP; regla de acceso garantizado de Codd; relación 1:N mediante clave foránea; formulación clásica de componentes de proceso del SGBD. Criterio: conservar conceptos de examen independientes de producto.

Cambio 04 — II.07: actualizados contenedores, CI/CD y Kubernetes.

Motivo/evidencia: preguntas oficiales 2019 y 2022 y OEP 2025 provisional; documentación oficial vigente.

Contenido: Kubernetes 1.37/1.36/1.35 a 22/09/2026; registry público/privado; volúmenes Docker; significado CI/CD; hostPID y namespace PID del host. Criterio: mantener arquitectura estable y verificar siempre versiones antes de reutilizar números concretos.

Cambio 05 — II.08: añadidas trampas recurrentes de redes.

Motivo/evidencia: preguntas oficiales 2022, 2024 y OEP 2025 provisional.

Contenido: sesión = diálogo/sincronización/recuperación; SNMP; clases IPv4 A/B/C como dato histórico de test, dejando claro que CIDR es el modelo actual; Best Effort/IntServ/DiffServ. Criterio: memorizar lo preguntado sin presentar classful como diseño moderno.

Cambio 06 — II.09: completado XML con organismos y tecnologías de enlace.

Motivo/evidencia: preguntas oficiales 2019 y 2022.

Contenido: W3C como responsable de XML; XLink = enlaces/relaciones entre recursos; regla XPath selecciona / XSLT transforma / XLink enlaza.

Cambio 07 — II.10–II.11: reforzados riesgos y auditoría ENS.

Motivo/evidencia: preguntas oficiales 2019, 2022 y 2024; RD 311/2022 vigente.

Contenido: integridad MAGERIT = completitud/corrección; mapa de calor probabilidad-impacto; auditoría ENS ordinaria al menos cada 2 años, posible extensión de 3 meses por fuerza mayor, y auditoría extraordinaria ante cambios sustanciales que reinicia el cómputo.

Cambio 08 — II.13: añadidos INÉS y precursor/indicador.

Motivo/evidencia: pregunta oficial 2024 y OEP 2025 provisional; documentación CCN vigente.

Contenido: INÉS como solución de gobernanza/evaluación ENS; precursor = indicio de incidente futuro, indicador = indicio de incidente ocurrido o en curso.

Cambio 09 — II.14: precisada propiedad intelectual y EUPL.

Motivo/evidencia: preguntas oficiales 2019, 2022 y OEP 2025 provisional; ENI y TRLPI vigentes.

Contenido: art. 97.4 TRLPI —derechos de explotación del software creado por trabajador en sus funciones corresponden al empresario salvo pacto contrario—; derechos exigidos por ENI para fuentes abiertas y regla «se procurará» EUPL.

Cambio 10 — II.15: añadidas dos trampas de evaluación multicriterio.

Motivo/evidencia: preguntas oficiales 2022.

Contenido: entropía como método de asignación/derivación de pesos, no de ordenación en la clasificación examinada; normalizaciones del material: ideal, máximo y suma, no fracción del mínimo.

Cambio 11 — II.16: actualizados estándares documentales, SEO y colaboración.

Motivo/evidencia: preguntas oficiales 2019 y 2022 y OEP 2025 provisional; fuentes oficiales vigentes.

Contenido: ISO 30300:2020; OAIS actualizado a ISO 14721:2025; CMIS 1.1 OASIS; SEO off-page = backlinks/menciones externas; archivos de canales estándar de Teams almacenados en el sitio SharePoint del equipo.

Cambio 12 — II.01: completados microdatos de almacenamiento magnético y sistemas distribuidos.

Motivo/evidencia: OEP 2024, plantilla definitiva (P29), y OEP 2025, plantilla provisional (P20).

Contenido: ZBR agrupa pistas en zonas y permite más sectores en las pistas exteriores; en sistemas distribuidos, la concurrencia global es una característica clásica y no existe un reloj global común.

Cambio 13 — II.03: completados literales oficiales de lenguajes y tipos.

Motivo/evidencia: OEP 2024, plantilla definitiva (P23-P24), y OEP 2025, plantilla provisional (P25).

Contenido: para PHP, INAP dio por válida en 2024 la combinación imperativo + orientación a objetos + funcional + reflexivo; en C, una constante no puede ser modificada por el programa; los tipos algebraicos permiten modelar de forma exhaustiva variantes/estados posibles.

Cambio 14 — II.04: reforzados OLAP y KPI con trampas oficiales.

Motivo/evidencia: OEP 2024, plantilla definitiva (P25-P26).

Contenido: tipos clásicos OLAP = ROLAP, MOLAP y HOLAP; “SOLAP = Spiral On-Line Analytical Processing” era la opción no válida. En KPI, la trampa oficial considera incorrecto que los objetivos puedan ser borrosos y no estar delimitados.

Cambio 15 — II.06: añadido middleware y ventaja de arquitectura multicapa.

Motivo/evidencia: OEP 2025, plantilla provisional (P29-P30).

Contenido: el middleware en sistemas distribuidos abstrae diferencias entre nodos y unifica la interfaz; la arquitectura multicapa reduce acoplamiento y facilita mantenimiento.

Cambio 16 — II.08: añadido cálculo de wildcard IPv4.

Motivo/evidencia: OEP 2025, plantilla provisional (P34).

Contenido: wildcard = inversa bit a bit de la máscara; /17 = 255.255.128.0 y wildcard 0.0.127.255. Una dirección 171.x pertenece históricamente a clase B en direccionamiento classful.

Cambio 17 — II.09: reforzada la distinción DTD/XSD.

Motivo/evidencia: OEP 2024, plantilla definitiva (P34).

Contenido: XML Schema (XSD) usa sintaxis XML, soporta tipos de datos y es fuertemente tipado frente a las limitaciones de DTD.

Cambio 18 — II.12: añadidos tipos de CRM y foco multicanal.

Motivo/evidencia: OEP 2024, plantilla definitiva (P39-P40).

Contenido: CRM operacional automatiza procesos de relación; CRM analítico explota datos; CRM colaborativo se centra en coordinar/mejorar la interacción con clientes a través de múltiples canales.

Cambio 19 — II.02: precisadas directivas de seguridad de Windows y concepto BYOD.

Motivo/evidencia: OEP 2024, plantilla definitiva (P21), y OEP 2025, plantilla provisional (P21). Vigencia contrastada con documentación actual de Microsoft y NIST SP 1800-22.

Contenido: las directivas de seguridad gestionan privilegios/derechos y opciones de seguridad en ámbito local y, mediante GPO/Active Directory, en dominio y controladores de dominio; BYOD significa uso de dispositivos personales para trabajo y añade retos específicos de seguridad y gestión.

Cambio 20 — II.13: añadidos alcance de SAT-INET/SAT-ICS y escala de peligrosidad CCN-STIC 817.

Motivo/evidencia: OEP 2024, plantilla definitiva (P41-P42), contrastada con documentación vigente del CCN-CERT y CCN-STIC 817.

Contenido: SAT-INET analiza el tráfico entre red interna e Internet y no se centra en contenido salvo lo relevante para detectar amenazas; SAT-ICS se orienta a redes de control/supervisión industrial; peligrosidad en cinco niveles: bajo, medio, alto, muy alto y crítico.

Cambio 21 — II.01: añadido microdato vigente de USB Type-C.

Motivo/evidencia: OEP 2018, plantilla definitiva (P21), contrastado con la especificación USB Type-C vigente de USB-IF en 2026.

Contenido: el conector USB Type-C dispone de 24 contactos/pines.

Cambio 22 — II.08: precisado el nivel de RTSP.

Motivo/evidencia: OEP 2018, plantilla definitiva (P32), y RFC 7826 vigente.

Contenido: RTSP (Real-Time Streaming Protocol) es un protocolo de capa de aplicación.

Cambio 23 — II.10-II.11: precisados impacto acumulado MAGERIT y trazabilidad.

Motivo/evidencia: OEP 2018, plantilla definitiva (P33-P34), contrastado con MAGERIT v3 y el marco ENS vigente.

Contenido: en MAGERIT, el impacto acumulado depende del valor acumulado y de la degradación; la trazabilidad permite imputar exclusivamente a una entidad las actuaciones realizadas por ella.

Cambio 24 — II.15: añadido Earned Value Management (EVM).

Motivo/evidencia: OEP 2018, plantilla definitiva (P41), reclasificada por su encaje directo en el tema actual de planificación, presupuestación y control de costes; vigencia contrastada con el estándar de EVM de PMI.

Contenido: EVM integra alcance, cronograma y coste para medir objetivamente el desempeño y progreso del proyecto.

## Cambio 25 — II.02: precisado el modelo clásico de microkernel.

## Motivo/evidencia: OEP 2025, plantilla provisional (P23); concepto estable y directamente encajado en fundamentos de sistemas operativos.

## Contenido: un microkernel mantiene en el núcleo solo mecanismos esenciales y desplaza servicios como sistemas de archivos, red o drivers a procesos/servidores en espacio de usuario, comunicados mediante paso de mensajes en un esquema de tipo cliente-servidor.

## Cambio 26 — II.03: completados Kotlin y carga dinámica de módulos Python.

## Motivo/evidencia: OEP 2019, plantilla definitiva (P23), y OEP 2025, plantilla provisional (P36); contrastado con documentación oficial vigente de Kotlin y Python 3.14.

## Contenido: Kotlin está diseñado para interoperar con el ecosistema Java/JVM; en Python, para importación programática se recomienda importlib.import\_module(), y SourceFileLoader sigue siendo un cargador de código fuente, aunque su método load\_module está deprecado.

## Cambio 27 — II.05: añadido ejemplo estable de SGBD NoSQL.

## Motivo/evidencia: OEP 2025, plantilla provisional (P28), contrastado con documentación oficial de Apache Cassandra 5.0.

## Contenido: Apache Cassandra es una base de datos distribuida NoSQL de modelo wide-column/columnas anchas, por lo que no debe confundirse con un SGBD relacional.

## Cambio 28 — II.08: recuperados microdatos de red antes clasificados en otro bloque.

## Motivo/evidencia: OEP 2019, plantilla definitiva (P80-P82 y P89); por el programa 2025 encajan directamente en direccionamiento/protocolos/operación de redes.

## Contenido: puertos de sistema o tradicionalmente well-known = 0-1023; una VLAN creada asignando puertos del switch es estática; en la clasificación clásica de monitorización de la pregunta, SNMP polling es activo y SNMP trap es pasivo; la IETF es la organización de estandarización de Internet cuyo producto técnico principal son los RFC.

## Cambio 29 — II.10: actualizadas referencias internacionales de gestión de riesgos.

## Motivo/evidencia: OEP 2025, plantilla provisional (P38), contrastada con ISO y NIST vigentes.

## Contenido: ISO/IEC 27005:2022 proporciona guía para la gestión de riesgos de seguridad de la información y NIST SP 800-30 Rev.1 es una guía para realizar evaluaciones de riesgos; se conserva MAGERIT como referencia AGE.

## Cambio 30 — II.13: recuperados C&C y smishing de preguntas reclasificadas.

## Motivo/evidencia: OEP 2019, plantilla definitiva (P87-P88); conceptos estables y directamente ligados al tema actual de ciberseguridad.

## Contenido: una infraestructura/servidor de Command and Control (C&C/C2) coordina o envía órdenes a sistemas comprometidos/bots; smishing es phishing realizado mediante SMS o mensajería de texto.

## Cambio 31 — control de actualidad a 23/09/2026.

## Motivo/evidencia: segunda pasada diferencial y comprobación de tecnologías de ciclo rápido.

## Contenido: Kubernetes continúa con ramas mantenidas 1.37/1.36/1.35, por lo que no requiere cambio doctrinal respecto de 22/09/2026; la fecha de fin de soporte de .NET 8 (10/11/2026), preguntada en OEP 2025 con plantilla provisional, se excluye como dato de ciclo de vida demasiado próximo a caducar y no se incorpora al temario estable.

## Control de exclusiones — auditoría 2018–2025

No incorporar automáticamente preguntas anuladas, datos caducados o trivia de versión/producto: Android 9 Pie, Qlik Sense, ANA/OpenText, VoiceXML 3.0 draft, Ryuk, detalles internos antiguos del planificador Windows, transitoria ENS de 24 meses, ERP/Moodle fuera del epígrafe actual, preguntas anuladas de PHP/registries, componentes coyunturales de FluxCD/ArgoCD y propuestas RSS no consolidadas. Además, la P27 de OEP 2024 sobre relación 1:N, anulada en la plantilla definitiva, no se usa como evidencia de examen; la asociación rígida «SSL = capa de sesión OSI» de la P33 OEP 2025, con plantilla solo provisional, no se incorpora como doctrina actual; y los requisitos concretos del equipo auditor de la antigua redacción de CCN-STIC 802 preguntados en OEP 2018 P35 no se conservan como vigentes, porque la guía de auditorías de cumplimiento fue actualizada y su esquema actual de requisitos no coincide con aquel literal.

No convertir una pregunta histórica en doctrina vigente si la norma, estándar, producto o guía ha cambiado. En CCN-STIC y software de ciclo rápido, verificar la versión oficial antes de conservar un microdato.

No incorporar como materia estable fechas de ciclo de vida de producto próximas a caducar. En particular, la OEP 2025 provisional preguntó el fin de soporte de .NET 8 LTS (10/11/2026): el dato seguía siendo correcto a 23/09/2026, pero caduca en pocas semanas y queda fuera del temario estable; si reaparece en una convocatoria futura deberá verificarse de nuevo. La plantilla OEP 2025 disponible es provisional: sus datos solo se mantienen cuando además han sido contrastados con documentación vigente o son conceptos técnicos estables.
...
* Kubernetes — documentación oficial de versiones: ramas mantenidas 1.37, 1.36 y 1.35 verificadas el 23/09/2026.
