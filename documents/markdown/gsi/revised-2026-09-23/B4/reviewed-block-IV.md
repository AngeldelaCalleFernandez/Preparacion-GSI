...
- AppLocker es una tecnología de control de aplicaciones de Windows: permite crear reglas para permitir o denegar la ejecución de ejecutables, scripts, instaladores, DLL y apps empaquetadas según usuario/grupo y atributos del archivo.

- Shadow IT = uso de dispositivos, aplicaciones o servicios tecnológicos no autorizados o no gestionados explícitamente por la organización; genera riesgos de inventario, seguridad, cumplimiento y soporte.

- En RHEL 9 la gestión de software se realiza con DNF; el comando yum se mantiene como alias/compatibilidad con generaciones anteriores. En Debian/Ubuntu, dpkg es el gestor de bajo nivel y APT gestiona paquetes/repositorios a nivel superior.

- Round Robin es planificación apropiativa por turnos: los procesos recorren una cola circular y reciben un quantum de CPU.

- El dirty bit de una página indica que ha sido modificada respecto a su copia en almacenamiento secundario y puede requerir escritura antes de reemplazarla.

- Kernel panic es una detención/estado de error crítico del kernel cuando el sistema no puede continuar de forma segura.

- LDIF (LDAP Data Interchange Format) es un formato textual estándar para representar/intercambiar entradas y cambios de directorios LDAP.
...
- Regla 3-2-1: mantener 3 copias de los datos, en 2 tipos/medios de almacenamiento distintos y 1 copia fuera del sitio/dominio de fallo. Las extensiones 3-2-1-1-0 añaden aislamiento/inmutabilidad y verificación.

- En RCM, el mantenimiento predictivo sólo tiene sentido cuando existe una condición/fallo potencial detectable y un intervalo P-F (Potential Failure → Functional Failure) identificable y monitorizable con tiempo para actuar.

- Deduplicación identifica bloques/objetos repetidos y conserva una única copia física o lógica de referencia, sustituyendo duplicados por referencias; no es simplemente compresión.
...
- Capacidad aproximada con discos iguales (o tomando el tamaño del menor): RAID 0 = N×S; RAID 1 clásico de dos discos ≈ S; RAID 5 = (N−1)×S; RAID 6 = (N−2)×S. La capacidad útil real puede ser menor por metadatos/formato.

- iSCSI transporta SCSI sobre TCP/IP; FCoE encapsula Fibre Channel sobre Ethernet. No inviertas ambas asociaciones.

- LUN (Logical Unit Number) identifica una unidad lógica SCSI presentada a un host; en SAN suele asociarse al volumen lógico que el host ve como dispositivo de bloque.

- La virtualización de almacenamiento puede implementarse en host/servidor, en la red/fabric o en la propia cabina; «shadow» no es una ubicación arquitectónica estándar.

- Amazon S3 es almacenamiento de objetos. Úsalo como ejemplo de servicio, no como definición de almacenamiento objeto.
...
- Hipervisor tipo 1 o bare-metal: se ejecuta directamente sobre el hardware; tipo 2 o hosted: se ejecuta sobre un sistema operativo anfitrión.

- HCI (Hyperconverged Infrastructure) integra de forma software-definida cómputo, almacenamiento y red/gestión en nodos de plataforma común.

- VDI centraliza escritorios virtuales; RDP es un protocolo habitual de acceso remoto en ecosistema Microsoft, no sinónimo universal de VDI.

- Weighted Least Connections selecciona teniendo en cuenta conexiones activas y peso/capacidad del backend; interesa la carga relativa al peso, no sólo el número bruto de conexiones.

- En modo passthrough/L4 el balanceador puede reenviar basándose en IP/puerto/conexión sin inspeccionar contenido HTTP ni terminar necesariamente TLS; el enrutamiento por URL/cookie/cabecera exige funciones L7.
...
- Una VLAN estática por puerto asigna puertos físicos del switch a una VLAN; no debe confundirse con asignación dinámica basada en identidad/política.

- IEEE 802.3x define control de flujo Ethernet full-duplex mediante tramas PAUSE.

- DHCP Snooping filtra/valida mensajes DHCP y permite marcar puertos confiables, ayudando a defender frente a servidores DHCP no autorizados/suplantación.

- Capas OSI de menor a mayor: Física, Enlace de datos, Red, Transporte, Sesión, Presentación y Aplicación.
...
- En monitorización SNMP, polling/GET es consulta activa periódica desde el gestor; TRAP es notificación asíncrona iniciada por el agente sin petición previa.

- SNMPv3 incorpora el User-based Security Model (USM), que aporta autenticación/integridad y puede aportar privacidad/cifrado según nivel configurado.

- RSVP reserva recursos por flujo de forma unidireccional; una comunicación en ambos sentidos requiere reservas independientes. QoS/RSVP no crea ancho de banda.
...
- En 802.11, DCF/CSMA-CA es el mecanismo básico de contienda; PCF no es obligatorio para que funcione CSMA/CA.

- WPA3 mantiene modalidades Personal y Enterprise; en empresa se combina habitualmente con 802.1X/EAP según diseño.

- Band Steering orienta clientes hacia una banda más conveniente (clásicamente 2,4→5 GHz; en redes modernas puede considerar también 6 GHz) según capacidades/política.

- NFC opera en 13,56 MHz. Bluetooth utiliza PHY basadas en GFSK; Bluetooth Low Energy está optimizado para menor consumo y no es simplemente «Bluetooth clásico más lento».

- Una cabecera MAC 802.11 puede contener hasta cuatro campos de dirección según To DS/From DS; no siempre se utilizan los cuatro.

- Channel bonding combina canales contiguos para formar anchos mayores; mejora pico de capacidad a costa de consumir más espectro.

- Wi-Fi 7/802.11be combina, entre otras mejoras, preamble puncturing para evitar subcanales interferidos y Multiple Resource Units (MRU) para asignar múltiples RU a una estación; esto permite aprovechar de forma flexible canales anchos.
...
- ICMP transporta mensajes de control y notificación de errores/alcanzabilidad asociados a IP; no es un protocolo de transporte de aplicación.

- El bloque multicast IPv4 es 224.0.0.0/4 (224.0.0.0–239.255.255.255).

- IPv6 encadena cabeceras de extensión mediante Next Header; entre las reconocibles están Hop-by-Hop Options, Routing, Fragment, Destination Options, AH y ESP. No existe una cabecera estándar llamada «Origin Options».

- El IETF desarrolla y publica buena parte de los estándares de Internet mediante RFC.

- CIDR permite direccionamiento sin clases y agregación/sumarización de prefijos, reduciendo rutas cuando los bloques son agregables.

- 6LoWPAN adapta IPv6 a redes personales inalámbricas de baja potencia, originalmente sobre IEEE 802.15.4, mediante mecanismos como compresión y fragmentación.
...
* Criterio: pregunta oficial → plantilla oficial → vigencia/validez 2026 → presencia real en V2.1 → parche mínimo. Las referencias a OEP 2025 se consideran provisionales mientras la plantilla disponible siga siendo provisional.
* Cambio 01 — IV.01 — Administración de SO y software base
* Origen: OEP 2018 Q70/Q82; OEP 2019 Q70; OEP 2024 Q73; OEP 2025 Q74-Q75 (provisional).
* Añadido: AppLocker; Shadow IT; DNF/YUM actualizado; Round Robin; dirty bit; kernel panic; LDIF.
* Depuración: no se incorpora como dato actual el límite de 8 caracteres de nombres JCL del examen 2018.
* Cambio 02 — IV.03 — Mantenimiento, incidencias y copias
* Origen: OEP 2024 Q77; OEP 2025 Q78-Q79 (provisional).
* Añadido: literal 3-2-1; intervalo P-F para mantenimiento predictivo RCM; deduplicación.
* Cambio 03 — IV.04 — Configuración, versiones, automatización y rendimiento
* Origen: OEP 2018 Q75; OEP 2019 Q74-Q75; OEP 2024 Q78-Q79; OEP 2025 Q80 (provisional).
* Añadido: estados Git modified/staged/committed; ITIL 4/SVS como dato de reconocimiento; benchmark sintético; ELK; Ansible Playbook.
* Cambio 04 — IV.05 — Almacenamiento
* Origen: OEP 2018 Q78; OEP 2019 Q77; OEP 2022 Q77-Q79; OEP 2025 Q81 (provisional).
* Añadido: capacidad RAID 0/1/5/6; iSCSI frente a FCoE; LUN; ubicaciones de virtualización de almacenamiento; S3 como almacenamiento de objetos.
* Cambio 05 — IV.06 — CPD, virtualización y balanceo
* Origen: OEP 2018 Q86-Q87; OEP 2022 Q81; OEP 2024 Q82; OEP 2025 Q82-Q83 (provisional).
* Añadido: hipervisor tipo 1/tipo 2; HCI; VDI/RDP como reconocimiento; Weighted Least Connections; passthrough L4.
* Cambio 06 — IV.07 — Medios y cableado
* Origen: OEP 2018 Q80; OEP 2019 Q78; OEP 2024 Q83-Q84.
* Añadido: pérdidas típicas en fibra; GEO; APC/UPC; CAT6/CAT6A.
* Depuración: no se incorpora la pregunta anulada de fibra de 2019 ni catálogos de bandas satelitales de baja rentabilidad.
* Cambio 07 — IV.08 — LAN
* Origen: OEP 2019 Q81; OEP 2024 Q93; OEP 2025 Q86/Q88 (provisional).
* Añadido: VLAN estática por puerto; 802.3x PAUSE; DHCP Snooping; siete capas OSI.
* Cambio 08 — IV.09 — Administración de red y SNMP
* Origen: OEP 2019 Q82; OEP 2022 Q86/Q100; OEP 2025 Q85 (provisional).
* Añadido: polling activo frente a traps asíncronas; SNMPv3 USM; RSVP unidireccional.
* Cambio 09 — IV.10 — WAN y routing
* Origen: OEP 2018 Q89-Q90; OEP 2022 Q88-Q89; OEP 2024 Q88-Q89; OEP 2025 Q89-Q90 (provisional).
* Añadido: TTL MPLS; Dijkstra/OSPF; RIP 15/16 y triggered updates; área stub OSPF; IS-IS sobre capa 2 frente a OSPF sobre IP; DLCI no pertenece a MPLS.
* Cambio 10 — IV.11 — Wi-Fi y radio
* Origen: OEP 2018 Q81/Q99-Q100; OEP 2019 Q97-Q98; OEP 2022 Q82-Q83/Q90; OEP 2025 Q84/Q92 (provisional).
* Añadido: PCF no obligatoria; WPA3 Personal/Enterprise; Band Steering; NFC 13,56 MHz; Bluetooth/BLE; cuatro direcciones 802.11; channel bonding; preamble puncturing + MRU en Wi-Fi 7.
* Cambio 11 — IV.12 — Seguridad en redes
* Origen: OEP 2018 Q91/Q93; OEP 2019 Q87-Q88; OEP 2022 Q87; OEP 2024 Q85/Q87/Q92; OEP 2025 Q93 (provisional).
* Añadido: DLP; IKE y asociaciones de seguridad IPsec; UTM; C&C; smishing; SIEM; DDoS; Nessus como escáner/evaluador de vulnerabilidades.
* Depuración: no se conserva como dato actual la calificación de Nessus como «open source» de la OEP 2024; el producto actual de Tenable se estudia por su función de evaluación/escaneo de vulnerabilidades.
* Cambio 12 — IV.13 — Internet e IoT
* Origen: OEP 2018 Q83-Q85; OEP 2019 Q89-Q90; OEP 2025 Q94-Q95 (provisional).
* Añadido: ICMP; multicast IPv4 224.0.0.0/4; cabeceras de extensión IPv6; IETF/RFC; CIDR/agregación; 6LoWPAN.
* Cambio 13 — IV.14 — NGN/IMS/VoIP
* Origen: OEP 2018 Q96-Q97; OEP 2019 Q93/Q100; OEP 2025 Q96-Q97 (provisional).
* Añadido: H.323 binario frente a SIP textual; SIP RFC 3261; SDP RFC 4566; G.729; Diameter para AAA en IMS.
* Cambio 14 — IV.15 — Móvil y UEM
* Origen: OEP 2019 Q95; OEP 2022 Q97; OEP 2024 Q97-Q98; OEP 2025 Q98 (provisional).
* Añadido: UMTS/WCDMA; LTE OFDMA en downlink y SC-FDMA en uplink; relación software/virtualización con network slicing; geofencing.
* Depuración: no se incorporan como actuales las bandas españolas de 3G de la OEP 2022, por ser un dato temporal sujeto a refarming.
* Cambio 15 — IV.02 — Administración de SGBD
* Origen: OEP 2019 Q71; OEP 2025 Q77 (provisional).
* Añadido: reconocimiento de SQL Server Always On Availability Groups como solución de alta disponibilidad/recuperación; distinción Oracle BLOB/BFILE como microdato de producto preguntado, manteniéndolo fuera del núcleo conceptual.
* Exclusiones generales de la auditoría
* - Preguntas anuladas de las plantillas definitivas: no generan contenido.
* - Datos de producto/certificación caducados o de baja rentabilidad: no se convierten en núcleo del V2.1.
* - TISPAN/NASS/RACS, ADSL G.Lite y otros detalles de redes legadas se conservan fuera del núcleo salvo necesidad histórica expresa.
* - OEP 2025: toda incorporación derivada de esa convocatoria queda registrada como provisional hasta disponer de plantilla definitiva.
